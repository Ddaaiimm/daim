...
class Lesson extends Model
{
    ...
    public static function isTimeAvailable($weekday, $startTime, $endTime, $class, $dosen, $lesson)
    {
        $lessons = self::where('weekday', $weekday)
            ->when($lesson, function ($query) use ($lesson) {
                $query->where('id', '!=', $lesson);
            })
            ->where(function ($query) use ($class, $dosen) {
                $query->where('class_id', $class)
                    ->orWhere('dosen_id', $dosen);
            })
            ->where([
                ['start_time', '<', $endTime],
                ['end_time', '>', $startTime],
    ...
    public function scopeCalendarByRoleOrClassId($query)
    {
        return $query->when(!request()->input('class_id'), function ($query) {
            $query->when(auth()->user()->is_dosen, function ($query) {
                $query->where('dosen_id', auth()->user()->id);
            })
                ->when(auth()->user()->is_mahasiswa, function ($query) {
