...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.lessons.update", [$lesson->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="dosen_id">{{ trans('cruds.lesson.fields.dosen') }}</label>
                <select class="form-control select2 {{ $errors->has('dosen') ? 'is-invalid' : '' }}" name="dosen_id" id="dosen_id" required>
                    @foreach($dosens as $id => $dosen)
                        <option value="{{ $id }}" {{ ($lesson->dosen ? $lesson->dosen->id : old('dosen_id')) == $id ? 'selected' : '' }}>{{ $dosen }}</option>
                    @endforeach
                </select>
                @if($errors->has('dosen'))
                    <div class="invalid-feedback">
                ...
                @endif
                <span class="help-block">{{ trans('cruds.lesson.fields.dosen_helper') }}</span>
            </div>
            <div class="form-group">
